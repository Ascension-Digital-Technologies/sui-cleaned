...
if (-not $libclang) {
  throw "Unable to find libclang*.dll. Install MSYS2 clang/libclang packages or run scripts\setup-windows.bat."
}
...
if ($parts -notcontains $libclangDir) {
  $env:Path = "$libclangDir;$usrBin;$env:Path"
}
