...
fn layout_ok(root: &Path, failed: &mut bool) -> io::Result<bool> {
    ...
    let move_vm = root.join("crates/execution/move/language/vm").is_dir();
    check(failed, "crates/execution/move/language/vm", move_vm);
    ok &= move_vm;
...
fn status(strict: bool) -> io::Result<u8> {
    ...
    check(&mut failed, "xtask member", file_contains(root.join("Cargo.toml"), "crates/runtime/apps/xtask")?);
    check(&mut failed, "cargo xtask alias", file_contains(root.join(".cargo/config.toml"), "xtask =")?);
    check(&mut failed, "xtask under crates/runtime", root.join("crates/runtime/apps/xtask/Cargo.toml").exists());
    ...
    check(&mut failed, "Windows dynamic libclang helper", file_contains(root.join("scripts/lib/windows-env.ps1"), "Get-CandidateToolDirs")?);
    check(&mut failed, "Cargo config avoids static libclang path", !file_contains(root.join(".cargo/config.toml"), "LIBCLANG_PATH")?);
    ...
    let move_core = root.join("crates/execution/move/language/core/move-core-types/Cargo.toml");
    if move_core.exists() {
