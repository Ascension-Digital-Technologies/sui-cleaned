...
diesel migration run                                                          \
  ...
  --migration-dir "$REPO/crates/runtime-pg-db/migrations"

diesel migration run                                                          \
  --database-url "postgres://postgres:postgrespw@localhost:$PORT"             \
  --migration-dir "$REPO/crates/runtime-indexer-alt-schema/migrations"

# Generate the schema.rs file, excluding framework tables and including the
# copyright notice.
diesel print-schema                                                           \
  --database-url "postgres://postgres:postgrespw@localhost:$PORT"             \
  --patch-file "$REPO/crates/runtime-indexer-alt-schema/schema.patch"             \
  > "$REPO/crates/runtime-indexer-alt-schema/src/schema.rs"
